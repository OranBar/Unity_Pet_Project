...
public static class GUIStyleState_ToStringCustomExtension
{
	public static string ToStringCustom(this UnityEngine.GUIStyleState style)
	{
		string result = "";
		result += "----------- Style ToString -----------";
		result += "\nstyle.background =" + style.background;
		result += "\nstyle.textColor =" + style.textColor;
		result += "\nstyle.scaledBackgrounds =" + style.scaledBackgrounds
			.Select((bg, i) => new { bg = bg, index = i })
			.Aggregate("", (agg, item) => agg += "	\nscaledBackground " + item.index + ": " + item.bg);
...
namespace ScriptInspector
{
	...
	public class FGListPopup : FGPopupWindow
	{
		...
		private static void LoadResources(FGTextEditor editor)
		{
			...
			//editor.styles.normalStyle.normal.textColor
			Debug.LogFormat("{0}: {1}", "listItemStyle.normal", listItemStyle.normal.ToStringCustom());
			Debug.LogFormat("{0}: {1}", "listItemStyle.onFocused", listItemStyle.onFocused.ToStringCustom());
			Debug.LogFormat("{0}: {1}", "listItemStyle.onNormal.background", listItemStyle.onNormal.background);
			Debug.LogFormat("{0}: {1}", "listItemStyle.onNormal.textColor", listItemStyle.onNormal.textColor);

		}
		/*
		private static void LoadResources_TonRan()
		{
			if (symbolIcons == null)
				LoadSymbolIcons();
			...
			listItemStyle = new GUIStyle
			{
				...
				onFocused = { background = selectedListItem },
				onNormal = { background = inactiveListItem, textColor = Color.black },
			};
		}
		
		public static FGListPopup Create_TonRan(Rect buttonRect, bool flipped)
		{
			if (listItemStyle == null)
			{
				LoadResources_TonRan();
			}
			...
			listItemStyle.onNormal.textColor = Color.black;
			...
			startAtCharacterIndex = 0;
			lineIndex = 0;

			//Dunno about this one
			tokenLeft = new SyntaxToken(SyntaxToken.Kind.EOF, "");
		...
		*/
		...
		private void OnGUI()
		{
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				{
					Debug.Log(">> 2 << " + "<b>" + filteredData[i].GetName() + "</b>");
